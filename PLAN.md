...
- **M6 — Observability depth (1 wk).** Alerts engine + channels, **Energy page (hourly/daily/monthly power & cost)**, Fabric panel, benchmarks-as-jobs UI, fleet time-range explorer. *Gate: alert fires on pulled agent cable; kWh rollups populate from watt samples.* **Gate outcome (2026-09-24):** live on the fleet — pulled the dgx1 agent: `seed-node-down` fired (critical, entity dgx1) after the 2-minute hold; agent restart → auto-resolve ("condition cleared", actor auto) with the full fired→resolved trail in alert_events; kWh rollups populated from real GPU watts (0.4487 kWh / 24 h); engine tick samples the node DIRECTORY so pulled agents are visible as down (bug found and fixed during the drill); WS toast broadcast + per-severity webhooks unit-tested.
- **M7 — Hardening & release (½–1 wk).** Backup/export, RUNBOOKS complete (agent, NAS, NCCL, client onboarding), Playwright suite, docs-check clean, coverage gate, v1.0.0 tag via semver skill. *Gate: fresh-machine install from README to first request < 30 min.* **Gate outcome (2026-09-24): PASSED — 9m 0s end-to-end.** Fresh clone on the real fleet → second compose project (isolated `config/`, port override, isolated node agent): clone+build+healthy **+4 s** (warm Docker layer cache; a cold build adds ~3–4 min), agent handshake **+8 s**, recipe register+probe **+8 s**, deployment start dispatched **+8 s**, vLLM boot (first-run sm_121 JIT autotune, ~4.5 min cold; ~40 s with the recipe's `cc-qwen-cache` volume warm) → deployment `healthy` (servedIdMatch) → keyed `POST /v1/chat/completions` returned a real completion (usage recorded). The drill produced and validated the README quickstart now shipped (nodes.json seed incl. required `createdAt`; recipe folder incl. the `MCTL` fallback for non-login PATHs — the agent spawns recipe verbs without `~/.local/bin` on PATH; pinned vLLM image tag; cache volume). Environment findings fixed along the way: a stale GLM worker holding 117 GiB unified memory blocked model start (stopped cleanly via the live supervisor), two dead agent processes on the fleet restarted, modelctl's local root is HOME-relative (isolated-HOME drills need `~/.config/modelctl` visible). Release tag v1.0.0 deferred by owner decision until M8 lands; coverage gate ≥75 % met (~84 %).
- **M8 — Chat UI (open-webui-class experience, final milestone).** Chat with any served model from the dashboard: conversation list + **folders that act as project context** (folder description injected into every conversation prompt), streaming markdown responses, conversation persistence in SQLite, **image attachments** routed to vision-capable deployments. Gateway telemetry applies — every chat request is a client like any other. *Gate: chat end-to-end with a served model; folder context persists and is injected; image attachment reaches a vision model.* **Gate outcome (2026-09-24): PASSED on the live fleet.** Chat store in SQLite (migration 4: folders/conversations/messages/attachments), streaming proxy over the gateway router with synthetic client `dashboard-chat`, folder context injected as a leading system message, image attachments gated to aliases marked `vision`. Live evidence: `qwen3-0.6b` served on dgx2 through the real supervisor (recipe registered + probed + started from the dashboard) → a chat turn streamed a real completion (`ttft 324 ms`, answer used a fact present ONLY in the folder description — "ORION-7" — proving injection); a capped second turn returned engine-reported usage (`114 in / 48 out`, `stream_options.include_usage`); both turns appear in `/api/analysis/traces` with client `dashboard-chat`; folder + description + messages survived a dashboard container restart (ADR-0003 persistence). Vision routing verified by integration tests + the Playwright chat spec (a real browser streams markdown, shows the folder banner, uploads an image which reaches the upstream as an `image_url` part). Coverage: `server/src/chat` 95.7 % lines.
